...
async function fallbackSearch(params: SearchParams): Promise<SearchResult> {
  ...
  if (deadlineAfter) where.submissionDeadline = { gte: new Date(deadlineAfter) };
  ...
  switch (sort) {
    case 'deadline_asc': orderBy = { submissionDeadline: 'asc' }; break;
    case 'deadline_desc': orderBy = { submissionDeadline: 'desc' }; break;
